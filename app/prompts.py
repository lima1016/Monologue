...
def build_translate_messages(language, text) -> list[dict]:
    ...
    실제 모델로 일본어 29줄을 세 번씩(87회) 돌려 잰 것이 이 모양을 정했다:
    - 규칙만: 47회가 중국어로 샘. 예시 대화를 더해도 44회 -- 예시만으로는 못 막는다.
    - 사용자 턴을 한국어 요청으로 감싸면 38회. 마지막 턴이 순수 일본어면 모델이
      그 문자권으로 끌려가는데, 감싸면 그 끌림이 약해진다.
    - 샌 답에 한 번 되묻기(build_translate_retry_messages)까지 하면 18회.
